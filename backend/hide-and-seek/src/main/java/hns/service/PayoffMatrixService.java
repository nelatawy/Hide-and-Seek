...
public class PayoffMatrixService {
    ...
    public int[][] buildPayoffMatrix1DProximity(List<HidingSpot> spots) {
        int n = spots.size();
        int[][] matrix = new int[n][n];
        for (int h = 0; h < n; h++) {
            for (int s = 0; s < n; s++) {
                if (h == s) {
                    matrix[h][s] = diagonalPayoff(spots.get(h).getDifficulty());
                } else {
                    ...
                    matrix[h][s] = (int) Math.round(offDiagonalPayoff(spots.get(h).getDifficulty()) * factor);
                }
            }
        }
        return matrix;
    ...
    public int[][] buildPayoffMatrix2DProximity(List<HidingSpot> spots) {
        int n = spots.size();
        int[][] matrix = new int[n][n];
        for (int h = 0; h < n; h++) {
            for (int s = 0; s < n; s++) {

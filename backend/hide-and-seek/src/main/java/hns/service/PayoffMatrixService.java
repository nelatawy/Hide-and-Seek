...
public class PayoffMatrixService {
    ...
    public List<List<Integer>> buildPayoffMatrix(List<HidingSpot> spots) {
        int n = spots.size();
    ...
    public List<List<Integer>> buildPayoffMatrix1DProximity(List<HidingSpot> spots) {
        int n = spots.size();
        List<List<Integer>> matrix = new ArrayList<>();
        for (int h = 0; h < n; h++) {
            List<Integer> row = new ArrayList<>();
            for (int s = 0; s < n; s++) {
                if (h == s) {
                    row.add(diagonalPayoff(spots.get(h).getDifficulty()));
                } else {
                    ...
                    row.add((int) Math.round(offDiagonalPayoff(spots.get(h).getDifficulty()) * factor));
                }
            }
            matrix.add(row);
        }
    ...
    public List<List<Integer>> buildPayoffMatrix2DProximity(List<HidingSpot> spots) {
        int n = spots.size();
        List<List<Integer>> matrix = new ArrayList<>();
        for (int h = 0; h < n; h++) {
            List<Integer> row = new ArrayList<>();
